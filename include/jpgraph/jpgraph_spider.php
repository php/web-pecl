...
// PUBLIC METHODS	
	...
	JpGraphError::Raise("Client side image maps not supported for SpiderPlots.");
    }
...
// PUBLIC METHODS
	...
	if( $s<0.1 || $s>1 )
	    JpGraphError::Raise("Spider Plot size must be between 0.1 and 1. (Your value=$s)");
	...
	switch( $densy ) {
	    ...
	    break;		
	    default:
		JpGraphError::Raise("SpiderPlot Unsupported Tick density: $densy");
	}
    }
	...
	if( $min < 0 ) 
	    JpGraphError::Raise("Minimum data $min (Spider plots only makes sence to use when all data points > 0)");
